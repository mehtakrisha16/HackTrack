...
const Events = () => {
  ...
  const allEvents = React.useMemo(() => {
    ...
    const staticEvents = getOpportunitiesByType('event').map(event => ({
      ...
      location: typeof event.location === 'string' ? event.location : (event.location?.city || 'TBD'),
    ...
    const scrapedEvents = realEvents.map(event => ({
      ...
      applicationLink: event.applicationLink || event.registrationLink,
      registrationLink: event.registrationLink || event.applicationLink,
      applyLink: event.applyLink,
      url: event.sourceUrl || event.applicationLink,
      sourceUrl: event.sourceUrl,
