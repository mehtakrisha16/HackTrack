import React, { useState, useContext } from 'react';
import { jwtDecode } from 'jwt-decode';
import { AppContext } from '../../context/AppContext';
import { toast } from 'react-hot-toast';
import './GoogleSignInButton.css';

const GoogleSignInButton = ({ onSuccess, onError, buttonText = "Continue with Google" }) => {
  const [isLoading, setIsLoading] = useState(false);
  const { setLoading } = useContext(AppContext);

  // Initialize Google OAuth when component mounts
  React.useEffect(() => {
    // Load Google OAuth script
    const loadGoogleScript = () => {
      if (window.google) {
        initializeGoogleAuth();
        return;
      }

      const script = document.createElement('script');
      script.src = 'https://accounts.google.com/gsi/client';
      script.async = true;
      script.defer = true;
      script.onload = initializeGoogleAuth;
      script.onerror = () => {
        console.warn('Google OAuth script failed to load');
      };
      document.head.appendChild(script);
    };

    const initializeGoogleAuth = () => {
      if (window.google && process.env.REACT_APP_GOOGLE_CLIENT_ID) {
        try {
          window.google.accounts.id.initialize({
            client_id: process.env.REACT_APP_GOOGLE_CLIENT_ID,
            callback: handleCredentialResponse,
            auto_select: false,
            cancel_on_tap_outside: true
          });
        } catch (error) {
          console.warn('Google OAuth initialization failed:', error);
        }
      }
    };

    loadGoogleScript();
  }, []);

  const handleCredentialResponse = async (response) => {
    setIsLoading(true);
    setLoading(true);
    
    try {
      // Send the credential token to our backend
      const backendResponse = await fetch('http://localhost:5000/api/auth/google', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          token: response.credential
        })
      });

      const data = await backendResponse.json();

      if (data.success) {
        // Store JWT token and user data
        localStorage.setItem('token', data.token);
        localStorage.setItem('user', JSON.stringify(data.user));
        
        // Call success callback
        if (onSuccess) {
          onSuccess(data);
        }
      } else {
        throw new Error(data.message || 'Google authentication failed');
      }
    } catch (error) {
      console.error('Google auth error:', error);
      if (onError) {
        onError(error.message || 'Google authentication failed');
      }
    } finally {
      setIsLoading(false);
      setLoading(false);
    }
  };

  // Demo function for presentation purposes
  const handleDemoGoogleSignIn = async () => {
    setIsLoading(true);
    setLoading(true);
    toast.success('Demo: Google OAuth in progress...');
    
    try {
      // Simulate Google OAuth success for demo
      const demoUser = {
        success: true,
        token: 'demo-jwt-token-' + Date.now(),
        user: {
          id: 'demo-google-user-' + Date.now(),
          name: 'Demo Google User',
          email: 'demo.google@example.com',
          profilePicture: 'https://lh3.googleusercontent.com/a/default-user=s96-c',
          preferences: {
            interests: ['Web Development', 'AI/ML'],
            skillLevel: 'intermediate',
            location: 'Mumbai'
          },
          isEmailVerified: true
        }
      };

      // Store demo data
      localStorage.setItem('token', demoUser.token);
      localStorage.setItem('user', JSON.stringify(demoUser.user));
      
      // Simulate network delay
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      if (onSuccess) {
        onSuccess(demoUser);
      }
      
      toast.success('Demo Google authentication successful!');
    } catch (error) {
      if (onError) {
        onError('Demo authentication failed');
      }
    } finally {
      setIsLoading(false);
      setLoading(false);
    }
  };

  const handleGoogleSignIn = () => {
    if (window.google && window.google.accounts && process.env.REACT_APP_GOOGLE_CLIENT_ID !== '1087338682831-c0f1l7e5tdl28k2fhh5bm9k8rq9v4k6s.apps.googleusercontent.com') {
      // Real Google OAuth
      window.google.accounts.id.prompt();
    } else {
      // Demo mode for presentation
      handleDemoGoogleSignIn();
    }
  };

  return (
    <button
      type="button"
      className="google-signin-button"
      onClick={handleGoogleSignIn}
      disabled={isLoading}
    >
      {isLoading ? (
        <div className="google-signin-loading">
          <div className="loading-spinner"></div>
          <span>Signing in...</span>
        </div>
      ) : (
        <div className="google-signin-content">
          <svg className="google-icon" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          <span>{buttonText}</span>
        </div>
      )}
    </button>
  );
};

export default GoogleSignInButton;